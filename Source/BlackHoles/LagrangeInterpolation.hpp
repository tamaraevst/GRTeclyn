...
private:    
    ...
    void build_stencil(amrex::Real grid_pos, int& base_idx, amrex::Real* weights)
    {
    int stencil[N];
    constexpr int center_offset = N / 2; // offset based on the number of points we are using 
    int center = static_cast<int>(amrex::Math::round(grid_pos)); // round the grid position to get the nearest cell center
