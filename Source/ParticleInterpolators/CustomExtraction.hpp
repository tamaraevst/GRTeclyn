...
//! extract values of a (possibly multiple-component) variable along a line (x-axis)
class CustomExtraction
{
  ...
  public:
    CustomExtraction(int a_start_comp, int a_ncomp, int a_num_points, double a_L,
                     std::array<double, AMREX_SPACEDIM> a_origin,
                     double a_dt, double a_time, bool a_restart_time, bool a_first_step)
        : m_start_comp(a_start_comp), m_ncomp(a_ncomp), m_num_points(a_num_points),
          m_L(a_L), m_origin(a_origin), m_dt(a_dt), m_time(a_time), m_restart_time(a_restart_time), m_first_step(a_first_step)
    {}

    ~CustomExtraction() = default;

    //! Execute using particle-based interpolation
    void execute_query(ParticleInterpolators &particles, std::string a_file_prefix) const
    {
        ...
        {
            interp_x[i] = m_origin[0] + (double(i) / double(m_num_points - 1)) * m_L;
        }
        ...
        for (int k = 0; k < m_ncomp; ++k)
        {
            ...
            query.addComp(m_start_comp + k, out_k,
                          Derivative::LOCAL, VariableType::state); // here state or derived? we will write stuff into out_k array
        }
        ...
        // fill the InterpolationQuery outputs from particle SOA with parity rules applied
        particles.interp(query);

        const bool first_step = (std::abs(m_time) < 0.1); // random for now, needs a fix
        SmallDataIO::Mode mode = first_step ? SmallDataIO::NEW : SmallDataIO::APPEND;
        ...
        else
        {
            for (int k = 0; k < m_ncomp; ++k)
            {
                ...
                output_file.write_time_data_line(std::vector<double>(row, row + m_num_points));
            }
        }
    }
};
