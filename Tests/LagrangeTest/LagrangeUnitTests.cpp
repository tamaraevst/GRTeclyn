...
void run_lagrange_test()
{
    ...
    amrex::Initialize(amrex_argc, amrex_argv);
    {
        ...
        amrex::FArrayBox out_fab(box, NUM_POLYNOMIAL_VARS,
                                 amrex::The_Managed_Arena()); //managed memory arena supports both CPU and GPU? 
    
        auto out_array      = out_fab.array(); //get the element-wise access to the data stored in the FArrayBox
        ...
        double expected_val = poly_test.compute_polynomial(x_interp, y_interp, z_interp);
        
        //Define a particle container: we have only one particle, so this is redundant, but will be useful for the future
        using MyParticleContainer = amrex::ParticleContainer<AMREX_SPACEDIM, 1>;
        MyParticleContainer particles;

        //Now, we are ready to set-up our particle. This requires defining: (i) geometry (ii) distribution mapping and (iii) a box array.
        
        //First, geometry:
        amrex::RealVect prob_lo{-0.5 * length};  
        amrex::RealVect prob_hi{+0.5 * length}; 
        amrex::RealBox real_box{prob_lo.dataPtr(), prob_hi.dataPtr()}; // real box is essentially the physical box, unlike just the 'box' in Amrex.
        ...
        auto& particle_tile = particles.DefineAndReturnParticleTile(lev, grid, tile);
        ...
        amrex::GpuArray<amrex::Real, 3> plo = {-0.5 * length, -0.5 * length, -0.5 * length};
        ...
        amrex::Print() << "Interpolated value is " << std::setprecision(cout_precision) << interp_val << "\n"
                       << "Expected value is " << std::setprecision(cout_precision) << expected_val << "\n"
                       << "Absolute error is " << std::setprecision(cout_precision) << error << "\n";
